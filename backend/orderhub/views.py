...
class ProductoViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # Verifica si el queryset tiene exactamente un objeto
        if queryset.count() == 1:
            instance = queryset.first()
            serializer = self.get_serializer(instance)
            return Response(serializer.data)
        
        # Si hay más de un resultado, devuelve una lista como de costumbre
...
class PedidoFilter(filters.FilterSet):
    mesa = filters.BooleanFilter(field_name='mesa', lookup_expr='exact')  # Para el campo 'cerrado'
    estado = filters.BaseInFilter(field_name='estado', lookup_expr='in')  # Filtro para múltiples valores
    cerrado = filters.BooleanFilter(field_name='cerrado', lookup_expr='exact')  # Para el campo 'cerrado'
...
class PedidoViewSet(ModelViewSet):
    ...
    ordering_fields = ['estado', 'fecha', 'hora']
    ordering = ['estado', 'fecha', 'hora']  # Ordena por defecto por estado_orden y luego por fecha_creacion
    ...
    def get_queryset(self):
        ...
        # Anotar el campo estado_orden para aplicar el orden personalizado
